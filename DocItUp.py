...
def test_env_var():
    ...
    if github_key:
        st.success(f"GitHub Token Loaded: {github_key[:4]}****")
    else:
        st.error("Failed to load GitHub Token.")

# Function to read the file from GitHub
def test_read_github_file():
    st.write("Testing GitHub File Read...")
    url = f"https://api.github.com/repos/{GITHUB_REPO}/contents/{FILE_PATH}"
    headers = {"Authorization": f"token {github_key}"}
    ...
    if response.status_code == 200:
        ...
        st.success(f"File read successfully! SHA: {sha}")
...
def test_update_github_file(new_content, sha):
    st.write("Testing GitHub File Update...")
    url = f"https://api.github.com/repos/{GITHUB_REPO}/contents/{FILE_PATH}"
    headers = {"Authorization": f"token {github_key}"}
    ...
    if response.status_code == 200:
        ...
        st.code(new_content, language='plaintext')
    else:
        st.error(f"Failed to update file. Status Code: {response.status_code}, Message: {response.text}")

# Function to test rate limits
def test_rate_limit():
    st.write("Testing GitHub API Rate Limits...")
    ...
    if response.status_code == 200:
        ...
        st.json(rate_limit_data)
        if rate_limit_data['rate']['remaining'] > 0:
            st.success(f"Remaining Requests: {rate_limit_data['rate']['remaining']}")
        else:
            st.error("Rate limit exceeded!")
    else:
        st.error(f"Failed to fetch rate limits. Status Code: {response.status_code}, Message: {response.text}")

# Streamlit App
def main():
    st.title("GitHub Integration Debugging")

    # Test environment variable loading
    test_env_var()

    # Read and display the current content from GitHub
    sha,data = test_read_github_file()
    ...
    if st.button("Check API Rate Limits"):
        test_rate_limit()
